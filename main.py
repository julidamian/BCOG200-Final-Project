...
class ImageProcessor: 
    ...
    def load_image(self, filepath, max_size=(800,600)): #resizing image to fit the screen
        image = Image.open(filepath).convert("L") #turning image into something that can be coloring in, monochromatic
...
class Display:
    ...
    #creating the frame the coloring book will work in
        self.canvas_frame = tk.Frame(self.root)
        self.canvas_frame.pack()

        self.image_canvas = tk.Canvas(self.canvas_frame, bg = "white", hightlightthickness=0)
        ...
        self.library_button = tk.Button(self.interface_frame, text="Choose This Image to Color", command=self.load_builtin_images)
    ...
    def paint(self, event):
        ...
        if self.drawing:
            self.image_canvas.create_oval(
                ...
                x + self.brush_size, y + size.brush_size,
